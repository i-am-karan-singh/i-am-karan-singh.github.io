...
\Description{Theoretical and applied Machine Learning, with a focus on \textbf {Reinforcement Learning} and \textbf{Dynamical Systems}. Generative Models, Online Learning, Learning with Partial Feedback, Optimization. }
...
\vspace{-0.25em}
\Description{\MarginText{Reinforcement Learning, Dynamical Systems} GPA: 4.0\ \ $\cdotp$\ \ \textit{PhD Candidate}\ \ $\cdotp$\ \ Computer Science\newline 
My research is focused on algorithms for machine learning with {\em provable guarantees} on computational and statistical efficiency, with an attentive emphasis on feedback-driven \textbf{interactive learning algorithms}. My prior research efforts have yielded provable methods for learning \textbf{Linear Dynamical Systems} (\textbf{Spotlight} at NeurIPS 2017, \textbf{Oral} at NeurIPS 2018) and designing controls for the same, despite the non-convex nature of the maximum likelihood problem. My recent works seek to address issues concerning task-agnostic \textbf{exploration} in Reinforcement Learning and those that arise when dealing with \textbf{continuous state} and action spaces.\\ \hfill \\
Awarded the \textbf{SEAS Award for Excellence} in 2018, the \textbf{Spotlight Prize} at the New York Academy of Sciences' 12$^{th}$ Annual Machine Learning Symposium, and the \textbf{ICML 2017 Travel Award}.
}
...
My coursework included 12 graduate-level courses on theoretical computer science, machine learning, and mathematics. My senior thesis details sketch-based algorithms for machine learning, and lower bounds in the streaming model.\\ \hfill \\
...
\vspace{-0.7em}
\Description{\MarginText{NeurIPS 2018 \textbf{Oral Present.}}Elad Hazan, Holden Lee, \textbf{Karan Singh}, Cyril Zhang and Yi Zhang. Spectral Filtering for General Linear Dynamical Systems. In the \textit{Advances in Neural Information Processing Systems 31 (NIPS), 2018}.}
\vspace{-0.7em}
